...
Also prints a summary table of all metrics.
...
def plot_framestack_ablation(
    ...
    ax.axhline(
        ...
        label=f"Memoryless ({baseline_mean:.1f})",
    )
    ax.errorbar(
        n_stacks, means, yerr=ses,
        marker="o", capsize=4, color="C1", label="FrameStack",
    )
    ax.axhline(
        recurrent_mean, linestyle="--", color="C2", alpha=0.8,
        label=f"Recurrent ({recurrent_mean:.1f})",
    )
