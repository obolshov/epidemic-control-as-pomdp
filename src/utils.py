...
def log_simulation_results(
    results: List[SimulationResult], log_dir: str = "logs"
) -> None:
    """
    ...
    for result in results:
        ...
        with open(log_path, "w", encoding="utf-8") as f:
            ...
            header = f"{'Day':<8} {'S':<14} {'I':<14} {'R':<14} {'Reward':<12} {'Action':<15}\n"
            f.write(header)
            f.write("-" * 90 + "\n")

            for i, (day, state, action, reward) in enumerate(
                zip(
                    result.action_timesteps,
                    result.action_states,
                    result.actions,
                    result.rewards,
                )
            ):
                row = f"{day:<8} {state.S:<14.2f} {state.I:<14.2f} {state.R:<14.2f} {reward:<12.4f} {action.name:<15}\n"
                f.write(row)
